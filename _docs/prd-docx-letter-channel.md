...
> Version: 1.5 — Updated 2026-03-29
> Status: Draft
...
| 1.5 | 2026-03-29 | **Implementor clarifications.** Specified exception layer ownership: `TemplateBodyNotFoundException` and `TemplateBodyCorruptedException` live in `Application/Interfaces/` (part of the `ITemplateBodyStore` contract). Specified F-307 trigger: manifest validation runs on upload only (non-blocking warning). Added optimistic concurrency guard to `Template` entity (EF rowversion) to prevent concurrent-upload race condition on `TemplateService.UpdateAsync` — concurrent update returns HTTP 409. Specified SHA-256 `BodyChecksum` computation timing: computed from the uploaded stream before writing to disk (single pass, no extra file read). Defined Phase 4 designer guide acceptance criteria: merged to `docs/letter-template-designer-guide.md` and explicitly approved by Product Owner before release. Added deployment note: EF down-migration is not provided for the Epic 1 storage migration — rollback requires restoring the database from backup. |
...
| F-105 | Atomic write + path commit | As the system, I need template file writes and database path commits to be atomic: write the file first, then commit the path to the DB; on DB failure, delete the orphaned file immediately in the same request. Periodic orphaned-file scanning (for files that survived despite a DB failure) is handled by an external background job — that job is out of scope for this initiative. **On update:** the current body file is **copied** (not moved) to `history/v{n}.docx` before the new version is written as `v{n+1}.docx`. Both files coexist on disk; TemplateHistory stores the history path. This ensures a complete audit record without relying on a file rename being atomic. **Concurrency guard:** The `Template` entity carries an EF Core `rowversion` concurrency token. `TemplateService.UpdateAsync` relies on EF's optimistic concurrency — if two concurrent requests attempt to update the same template, the second `SaveChangesAsync` throws `DbUpdateConcurrencyException`, which the service translates to HTTP 409 Conflict. This prevents racing writes to `v{n+1}.docx`. | Must-have |
...
| F-307 | Manifest validation for DOCX | As Claire, I want the system to warn me if my DOCX contains undeclared placeholders before I publish. **Trigger:** validation runs at upload time only (non-blocking — the file is saved even if warnings are present). The API response for `POST /api/templates/letter` and `PUT /api/templates/{id}/letter` includes a `warnings` array listing any `{{ key }}` placeholders found in the DOCX that are not declared in the template manifest. No hard block at publish time for undeclared placeholders. | Must-have |
...
- EF Core migration: replace `HtmlBody` / `DocxBody` columns with `BodyPath` (nvarchar) on `Templates` and `TemplateHistory`. Add `RowVersion` (timestamp) concurrency token to `Template`. No data migration needed (never in production). **Deployment note:** A down-migration (rollback) is not provided — if rollback is required, restore the database from backup before re-deploying the previous version.
...
- Designer guide: `docs/letter-template-designer-guide.md` — supported Word features, placeholder syntax (`{{ key }}`), collection table pattern, conditional pattern, smart-quote note, unsupported features list. **Done criteria:** file merged to `docs/letter-template-designer-guide.md` in the main branch **and** explicitly approved by the Product Owner before release. Approval is a release gate — the feature does not ship without it.
