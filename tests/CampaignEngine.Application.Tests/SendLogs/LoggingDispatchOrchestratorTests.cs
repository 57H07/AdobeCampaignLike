...
public class LoggingDispatchOrchestratorTests
{
    ...
    private LoggingDispatchOrchestrator BuildOrchestrator() =>
        new(_registry.Object, _sendLogService.Object, _logger.Object);
    ...
    // ----------------------------------------------------------------
    // Business rule: status updated to Retrying on transient failure
    // ----------------------------------------------------------------

    [Fact]
    public async Task SendWithLogging_TransientFailure_LogsRetrying()
    {
        // Arrange
        var sendLogId = Guid.NewGuid();
        var request = BuildRequest();
        ...
        // Sent/Failed should NOT be called
        _sendLogService.Verify(
    ...
    [Fact]
    public async Task SendWithLogging_DispatcherThrows_LogsRetryingWithException()
    {
        // Arrange
        var sendLogId = Guid.NewGuid();
        var request = BuildRequest();
        ...
        _sendLogService.Verify(
            s => s.LogRetryingAsync(sendLogId, It.Is<string>(e => e.Contains("SMTP server unreachable")), 1, default),
            Times.Once);
    }
    ...
    [InlineData(1, 2)]
    [InlineData(2, 3)]
    public async Task SendWithLogging_TransientFailure_IncrementsRetryCount(
        int currentRetry, int expectedRetryCount)
    {
        ...
        // Assert
        _sendLogService.Verify(
            s => s.LogRetryingAsync(sendLogId, "Timeout", expectedRetryCount, default),
            Times.Once);
    }
}
